...
def single_render(provider: str) -> None:
    ...
    if provider == "gitlab":
        configuration = get_configuration(
            config=settings.REVIEWS_GITLAB_REPOSITORY_CONFIGURATION
        )
        body = GitlabPullRequestController().render(configuration=configuration)
    else:
        configuration = get_configuration(
            config=settings.REVIEWS_GITHUB_REPOSITORY_CONFIGURATION
        )
    ...
    configurations = [
        {
            ...
            if show
            else "".join("*" for _ in range(len(settings.GITHUB_TOKEN))),
        },
        ...
        {"name": "GITHUB_URL", "value": settings.GITHUB_URL},
        {
            ...
            if show
            else "".join("*" for _ in range(len(settings.GITLAB_TOKEN))),
        },
